...
async def upload_to_cloudinary(file: UploadFile, folder: str = "logos") -> dict:
    ...
    try:
        upload_result = cloudinary.uploader.upload(contents, folder=f"faturaxpress/{folder}", public_id=public_id, resource_type="image", overwrite=True)
    ...
    return {
        "original_url": upload_result["secure_url"], "optimized_url": optimized_url, "public_id": upload_result["public_id"],
        "width": upload_result["width"], "height": upload_result["height"], "format": upload_result["format"],
    }
