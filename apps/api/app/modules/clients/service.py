...
def validar_nif_local(db: Session, nif: str, company_id: uuid.UUID):
    ...
    # 999999999 é consumidor final - sempre permite novo
...
def create_cliente(db: Session, cliente: ClienteCreateRequest, company_id: uuid.UUID):
    ...
    # BLOQUEIA SE JA EXISTE - exceto 999999999
    if nif_clean != "999999999":
        exists = db.query(Cliente).filter(
            Cliente.nif == nif_clean,
        ...
        if exists:
            raise HTTPException(
                status_code=400,
                detail=f"Já existe um cliente com este NIF: {exists.nome} ({exists.nif})"
            )
...
def delete_cliente(db, cliente_id, company_id):
    ...
    if db.query(Fatura).filter(Fatura.cliente_id == cliente_id).first():
        raise HTTPException(status_code=400, detail="Não pode apagar cliente com faturas emitidas")
