...
def _gerar_pdf_reportlab(doc: DocumentoGerado, empresa, funcionario=None):
    ...
    emp_nif = get_val(["nif"], "")
    emp_end = get_val(["address","endereco"], "")
    emp_tel = get_val(["phone","telefone"], "")
    emp_email = get_val(["email"], "")
    emp_cidade = get_val(["city","cidade"], "Luanda")
    ...
    if raw_logo:
        try:
            if raw_logo.startswith("http"):
                ...
                _, b64 = raw_logo.split(",",1)
                logo_bytes = base64.b64decode(b64)
            elif os.path.exists(raw_logo):
                with open(raw_logo, "rb") as f:
                    logo_bytes = f.read()
            if logo_bytes:
                # salva temp pra platypus
    ...
    if logo_temp_path:
        logo_img = RLImage(logo_temp_path, width=110, height=90, kind='proportional')
        t = Table([[logo_img, emp_info]], colWidths=[110, 380])
    else:
        t = Table([[Paragraph(f"<b>{emp_nome[0] if emp_nome else 'E'}</b>", s_bold), emp_info]], colWidths=[110,380])
    ...
    titulo = Paragraph(f"<b>CONTRATO DE TRABALHO POR TEMPO INDETERMINADO</b><br/><font size=7 color=#777>LGT Angola</font>", s_normal)
    codigo = Paragraph(f"<b>{doc.codigo_verificacao}</b><br/><font size=7 color=#777>ORIGINAL</font>", s_small)
    ...
    # GRID igual fatura
    boxes = [[
        Paragraph(f"<b>FUNCIONARIO</b><br/>{vars_p.get('nome_funcionario','---')}", s_small),
        Paragraph(f"<b>BI</b><br/>{vars_p.get('bi','---')}", s_small),
        Paragraph(f"<b>CARGO</b><br/>{vars_p.get('cargo','---')}", s_small),
        Paragraph(f"<b>ADMISSÃO</b><br/>{vars_p.get('data_admissao','---')}", s_small),
        Paragraph(f"<b>SALÁRIO</b><br/>{vars_p.get('salario_base_formatado','---')}", s_small)
    ]]
    bt = Table(boxes, colWidths=[110,90,80,90,120])
    bt.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,colors.HexColor("#bbb")),('ALIGN',(0,0),(-1,-1),'CENTER')]))
    story.append(bt); story.append(Spacer(1,10))

    # clausulas limpas
    import re
    html = doc.conteudo_html_final or ""
    # pega só texto entre <p>
    ps = re.findall(r'<p[^>]*>(.*?)</p>', html, re.IGNORECASE|re.DOTALL)
    for p in ps:
        clean = re.sub(r'<[^>]+>', '', p).strip()
        if clean:
            story.append(Paragraph(clean, s_normal))
...
def pdf_documento(documento_id: uuid.UUID, request: Request, token: Optional[str] = Query(None), db: Session = Depends(get_db), auth = Depends(get_company_from_request)):
    ...
    # Tenta ReportLab primeiro (igual fatura) - nunca quebra Jinja
    try:
        ...
        print(f"[PDF ReportLab falhou, tenta Weasy] {e}")
        try:
            ...
            raise HTTPException(500, f"Erro PDF: {e2}")
