...
from app.modules.documentos.models import ModeloDocumento
...
def seed_modelos(db: Session, company_id: uuid.UUID):
    # Verifica se já tem
    if db.query(ModeloDocumento).filter(ModeloDocumento.company_id == company_id, ModeloDocumento.is_sistema == True).first():
        print("Modelos padrão já existem, pulando seed")
        return
    ...
    modelos = [
        {
            ...
            "tipo": "contrato_efetivo",
            "categoria": "admissao",
            "descricao": "Contrato por tempo indeterminado - Lei Geral do Trabalho Angola",
    ...
    for m in modelos:
        doc = ModeloDocumento(
            ...
            variaveis_usadas=["nome_funcionario", "bi", "nome_empresa"],
            versao=1,
            is_ativo=True,
        ...
        db.add(doc)
    db.commit()
    print(f"✅ {len(modelos)} modelos padrão criados")
