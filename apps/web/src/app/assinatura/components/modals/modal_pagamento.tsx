...
export default function PagamentoModal({ open, checkoutInfo, onClose, onSuccess, apiBase }: Props) {
  ...
  // trava ESC e scroll quando aberta
  useEffect(() => {
    if (open) {
  ...
  const handleSend = async () => {
    ...
    if (!allowed.includes(file.type)) return toast.error('Use JPG, PNG ou PDF', { position: 'top-center' })
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.detail || 'Erro ao enviar comprovativo')
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      {/* backdrop travado - clique não fecha */}
        ...
        <div className="px-6 pb-6 pt-3 overflow-auto flex-1 flex flex-col gap-[10px]">
          <div className="flex flex-col gap-[2px]">
            ...
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-[12px] text-[11.5px] text-amber-800 leading-relaxed">
              ...
              Coloca a referência <b>{checkoutInfo.subscription.reference}</b> na descrição da transferência
            </div>
          </div>
          ...
          <div className="flex gap-3 mt-6">
            <button type="button" onClick={onClose} disabled={loading} className="flex-1 h-11 rounded-full border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 flex items-center justify-center gap-2 disabled:opacity-50"><X className="w-4 h-4" /></button>
            <button type="button" onClick={handleSend} disabled={loading ||!file} className="flex-1 h-11 rounded-full bg-[#0095ff] text-white font-semibold hover:bg-[#0085e6] shadow-[0_6px_20px_rgba(0,149,255,0.35)] flex items-center justify-center gap-2 disabled:opacity-50">
              {loading? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-5 h-5" />}{loading? 'Enviando...' : 'Enviar comprovativo'}
            </button>
          </div>
          <p className="text-[10px] text-gray-400 text-center mt-2">Validação manual em até 30min. Hash verificado contra reuso.</p>
        </div>
      </div>
    </div>
  )
