    ...
    <div className="min-h-screen bg-white relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-4 sm:px-8 py-6">
        ...
        <div className="text-center max-w-[600px] mx-auto mb-8">
          ...
          <h1 className="text-[32px] sm:text-[44px] font-bold leading-[0.9] tracking-tight">
          ...
          {PLANS.map((plan, idx) => {
            ...
            return (
              <div
                id={`card-${idx}`}
                key={plan.id}
                className={`snap-center shrink-0 w-[88%] md:w-auto relative rounded-[24px] bg-[#0f0f0f] border p-5 flex flex-col min-h-[400px] transition-all duration-300 hover:-translate-y-1
                  ${plan.popular? 'border-[#ff2d87]/50 shadow-[0_20px_60px_rgba(255,45,135,0.25)] md:scale-[1.02]' : 'border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.1)]'}
                `}
              >
                <div className="flex flex-col items-center text-center">
                  <div className={`w-8 h-8 rounded-[9px] flex items-center justify-center mb-4 ${plan.popular? 'bg-gradient-to-br from-[#ff0099] to-[#ff7ac4]' : 'bg-[#2a2a2a]'}`}>
                    <Icon className="w-4 h-4 text-white" />
                  </div>
                  <h3 className="text-[17px] font-extrabold text-white tracking-wide">{plan.name}</h3>
                  <p className="text-[11px] text-white/50 mt-0.5">{plan.sub}</p>
                </div>

                <div className="mt-4 flex items-baseline justify-center gap-1">
                  <span className="text-[10px] text-white/60">Kz</span>
                ...
                <div className="mt-5 space-y-2.5 flex-1">
                  {plan.features.map((f, i) => (
                    <div key={i} className="flex items-center gap-2.5 text-[12px] text-white/80">
                ...
                <button
                  ...
                  className={`mt-5 w-full h-[40px] rounded-full text-[12px] font-bold transition
                    ${plan.popular? 'bg-gradient-to-r from-[#ff0055] to-[#ff3ac0] text-white shadow-[0_6px_18px_rgba(255,0,135,0.35)]' : 'bg-[#2a2a2a] text-white hover:bg-[#333]'}
                  `}
                >
