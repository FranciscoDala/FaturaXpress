...
// FIX 404: teu main.py registra com /api, então usa /api/assinatura
const ASSINATURA_BASE = `${API_URL}/api/assinatura`
...
function SkeletonCard() {
  return <div className="snap-center shrink-0 w-[88%] md:w-auto rounded-[24px] bg-white border border-black/[0.06] p-5 min-h-[400px] animate-pulse"><div className="w-8 h-8 bg-gray-200 rounded-[9px] mx-auto mb-4" /><div className="h-4 w-20 bg-gray-200 rounded mx-auto" /></div>
}
...
export default function AssinaturaPage() {
  ...
  const handleSubscribe = async (plan: any) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(body.detail || body.msg || 'Erro ao criar checkout')

      const infoRes = await fetch(`${ASSINATURA_BASE}/checkout/${body.subscription_id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (!infoRes.ok) throw new Error('Checkout criado mas falha ao buscar dados de pagamento')
            ...
            return (
              <div id={`card-${idx}`} key={plan.id} className={`snap-center shrink-0 w-[88%] md:w-auto rounded-[24px] bg-white border p-5 flex flex-col min-h-[400px] hover:-translate-y-1 transition-all ${style.popular || plan.popular? 'border-[#ff2d87]/30 shadow-[0_20px_60px_rgba(255,45,135,0.18)] md:scale-[1.02]' : 'border-black/[0.06] shadow-[0_8px_30px_rgba(0,0,0,0.06)]'}`}>
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="text-[17px] font-extrabold tracking-wide">{plan.name}</h3><p className="text-[11px] text-gray-500 mt-0.5">{plan.sub}</p>
                </div>
                ...
                <button onClick={() => handleSubscribe(plan)} disabled={!!loading} className={`mt-5 w-full h-[40px] rounded-full text-[12px] font-bold transition disabled:opacity-50 ${style.btn}`}>{loading === plan.id? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : plan.price_raw === 0? 'Plano atual' : 'Assinar agora'}</button>
              </div>
            )
          })}
        </div>
