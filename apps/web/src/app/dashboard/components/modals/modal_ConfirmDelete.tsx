        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[24px] w-full max-w-[400px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.25)]">
                <div className="relative h-[72px] bg-[#FFE9E9] px-5 pt-5 flex justify-between items-start">
                    <div className="w-9 h-9 rounded-full bg-white border shadow-sm flex items-center justify-center"><AlertTriangle className="w-4 h-4 text-red-500" /></div>
                ...
                <div className="px-6 pt-5 pb-6">
                    ...
                    <p className="text-[13.5px] text-gray-500 mt-3 leading-relaxed">{description} {itemName && <span className="font-bold text-gray-800">"{itemName}"</span>} será removido permanentemente.</p>
                    <div className="flex gap-3 mt-8">
                        ...
                        <button onClick={onConfirm} disabled={loading} className="flex-1 h-11 rounded-full bg-[#FF3B30] text-white font-semibold hover:bg-red-600 shadow-[0_6px_20px_rgba(255,59,48,0.35)] flex items-center justify-center gap-2 disabled:opacity-50">{loading? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Trash2 className="w-5 h-5" />}</button>
                    </div>
                </div>
            </div>
        </div>
