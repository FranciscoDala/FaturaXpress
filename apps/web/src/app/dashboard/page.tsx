...
interface Produto {
    id: string; nome: string; codigo: string; categoria: string | null
    preco_venda: number | string; stock_atual: number; unidade: string
    imagem_url: string | null; ativo: boolean; descricao?: string | null
    tipo?: 'produto' | 'servico' | 'kit'; controlar_stock?: boolean; iva?: number
}
...
export default function DashboardPage() {
    ...
    // Fix TS sem alterar o modal_Produto.tsx - converte para any
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <ClienteModal
                ...
                onSuccess={() => { toast.success(clienteSelecionado? 'Atualizado' : 'Criado'); fetchClientes() }}
            />
            <ProdutoModalAny
                open={modalProdutoOpen}
                produto={produtoSelecionado}
                onClose={() => setModalProdutoOpen(false)}
                onSuccess={() => { toast.success(produtoSelecionado? 'Produto atualizado' : 'Produto criado'); fetchProdutos() }}
            />
