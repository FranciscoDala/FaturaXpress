...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        api.get('/api/auth/me').then(r => setEmpresa(r.data.company || r.data)).catch(() => { })
        fetchFaturasGeral()
    }, [])
                            ...
                            <div className="flex flex-row justify-between items-start gap-4 w-full">
                                <div className="flex flex-col items-start text-left">
                                    ...
                                    <div className="mt-3 space-y-1 text-[13px] text-[#4a5568] text-left">
                                        ...
                                        <p className="text-[11px] text-gray-500">Controlo geral - {faturasCurso.length + faturasEmitidas.length} docs</p>
                                    </div>
                                </div>
                        ...
                        <>
                            ...
                            <div className="mt-6 bg-white rounded-[22px] p-6 border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="bg-[#F7F8FA] rounded-[16px] p-4 border border-gray-100"><p className="text-[11px] text-gray-500">Total Faturado FT</p><p className="text-[18px] font-bold mt-1">{faturasEmitidas.reduce((s: any, f: any) => s + Number(f.total_geral || f.total || 0), 0).toFixed(2)} KZ</p></div>
                                    <div className="bg-[#F7F8FA] rounded-[16px] p-4 border border-gray-100"><p className="text-[11px] text-gray-500">Faturas Emitidas</p><p className="text-[18px] font-bold mt-1">{faturasEmitidas.length}</p></div>
                                    <div className="bg-[#F7F8FA] rounded-[16px] p-4 border border-gray-100"><p className="text-[11px] text-gray-500">Proformas</p><p className="text-[18px] font-bold mt-1">{faturasCurso.length}</p></div>
                                </div>
                            </div>
                        </>
                    )}
