...
import { Package, ChevronDown, Users, FileDown, Check, Power, Receipt, Pencil, Database, Search, Crown } from 'lucide-react'
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        const currView = localStorage.getItem(LS_KEYS.view)
        const currFtab = localStorage.getItem(LS_KEYS.ftab)
        const currList = localStorage.getItem(LS_KEYS.list)
        if (currView === homeView && currFtab === faturaTab && currList === listView) return
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-4 items-start text-left">
                        ...
                        <div className="flex-1 w-full min-w-0">
                            ...
                            <div className="mt-5 flex max-w-[520px] w-full bg-white/80 backdrop-blur border rounded-[3px] overflow-hidden shadow-sm">
                                <button
                                    onClick={() => { setHomeView('faturas'); setFaturaTab('curso') }}
                                    className={`flex-1 py-2.5 flex flex-col justify-center items-center text-center border-r border-gray-200 transition
        ${homeView === 'faturas' && faturaTab === 'curso' ? 'bg-[#F0F7FF] text-[#0095ff]' : 'bg-white text-gray-800 hover:bg-gray-50'}`}
                                >
                                    <p className="text-[15px] font-bold leading-none">{loadingFaturas ? '...' : faturasCurso.length}</p>
                                    <p className="text-[11px] text-gray-500 mt-[2px]">Proforma PP</p>
                                </button>
                                <button
                                    onClick={() => { setHomeView('faturas'); setFaturaTab('emitidas') }}
                                    className={`flex-1 py-2.5 flex flex-col justify-center items-center text-center transition
                                        ${homeView === 'faturas' && faturaTab === 'emitidas' ? 'bg-[#F0F7FF] text-[#0095ff]' : 'bg-white text-gray-800 hover:bg-gray-50'}`}
                                >
                                    <p className="text-[15px] font-bold leading-none">{loadingFaturas ? '...' : faturasFTOnly.length}</p>
