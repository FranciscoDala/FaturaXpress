...
import { MapPin, Plus, Trash2, Search, Star, XCircle, FileText } from 'lucide-react'
...
export default function EmitirFaturaPage() {
  ...
  const emitir = async()=>{
    ...
    try{ await api.post('/api/faturas',{ cliente_id:clienteId, itens: itens.map(i=>({produto_id:i.produto_id, quantidade:i.quantidade, preco_unit:i.preco_unit}))}); toast.success('Fatura emitida'); setItens([]); setActiveTab('curso'); fetchFaturas() }
  ...
  const handleApagar = async()=>{
    try{ await api.delete(`/api/faturas/${deleteTarget.id}`); toast.success('Apagada'); setDeleteTarget(null); fetchFaturas() }catch{ toast.error('Erro ao apagar') }
  }
      ...
      <div className="max-w-[1100px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start gap-4">
                ...
                <button onClick={()=>navigate('/app/dashboard')} className="bg-[#FF3B30] hover:bg-red-600 text-white text-[12px] font-semibold px-4 py-1.5 rounded-full shadow">
                  Voltar
                </button>
              </div>
              ...
              <div className="mt-6 flex bg-white border border-gray-200 rounded-[3px] overflow-hidden max-w-[520px]">
                ...
                <button onClick={()=>setActiveTab('emitir')} className={`flex-[1.2] text-[13px] font-semibold ${activeTab==='emitir'? 'bg-[#0095ff] text-white' : 'bg-[#8ecfff] text-white hover:bg-[#6cc0ff]'}`}>
                  + Emitir Fatura
                </button>
              </div>
            </div>
              ...
              {itens.length===0? <span className="text-[11px] text-gray-400">Nenhum produto</span> :
                itens.map(it=>(
                  <span key={it.produto_id} className="bg-[#ff7a00] text-white text-[10px] font-bold px-2.5 py-1 rounded-[3px] flex items-center gap-1">
                    {it.nome.toUpperCase()} x{it.quantidade} <Star className="w-3 h-3 fill-white"/>
                  </span>
                ))
              }
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.8fr_1fr] gap-4">
              <div className="bg-white border border-gray-200 p-5">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-[12px] font-bold">Adicionar Produtos</h3>
                  <span className="text-[10px] text-gray-400">{produtos.length} produtos</span>
                </div>
                <div className="relative mb-3">
                  <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/>
                  <input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar produto..." className="w-full pl-9 pr-3 py-2 border rounded text-[13px]"/>
                </div>
                ...
                <div className="space-y-2">
                  {itens.map(it=>(
                    <div key={it.produto_id} className="flex gap-2 items-center text-[11px]">
                      <input type="number" min={1} value={it.quantidade} onChange={e=>{ const q=Number(e.target.value); setItens(prev=>prev.map(x=> x.produto_id===it.produto_id? {...x, quantidade:q, subtotal:q*x.preco_unit}:x))}} className="w-10 border rounded px-1 py-0.5"/>
        ...
        {activeTab==='curso' && (
          <div className="mx-4 sm:mx-8 lg:mx-12 mt-4 bg-white border border-gray-200 p-5">
            <h3 className="text-[12px] font-bold mb-4">Faturas em Curso - podem ser canceladas e apagadas</h3>
            <div className="space-y-2">
              {faturasCurso.length===0? <p className="text-[12px] text-gray-400 text-center py-8">Nenhuma fatura em curso</p> :
                faturasCurso.map(f=>(
                  <div key={f.id} className="flex justify-between items-center border rounded px-3 py-2.5">
                    <div><p className="text-[13px] font-medium">{f.numero || f.id.slice(0,8)} - {f.total?.toFixed(2) || '0.00'} KZ</p><p className="text-[11px] text-gray-500">{f.status} • {new Date(f.created_at).toLocaleDateString()}</p></div>
          ...
          <div className="mx-4 sm:mx-8 lg:mx-12 mt-4 bg-white border border-gray-200 p-5">
            <div className="flex justify-between items-center mb-4">
              ...
              <select value={filtroStatus} onChange={e=>setFiltroStatus(e.target.value)} className="text-[11px] border rounded px-2 py-1">
                ...
                <option value="rascunho">Em Curso</option>
              </select>
            </div>
            <div className="space-y-2">
              {faturasFiltradas.length===0? <p className="text-[12px] text-gray-400 text-center py-8">Nenhuma fatura</p> :
                faturasFiltradas.map(f=>(
                  <div key={f.id} className="flex justify-between items-center border rounded px-3 py-2.5">
                    <div className="flex gap-2 items-center"><FileText className="w-4 h-4 text-gray-400"/><div><p className="text-[13px] font-medium">{f.numero || f.id.slice(0,8)} - {f.total?.toFixed(2) || '0.00'} KZ</p><p className="text-[11px] text-gray-500 capitalize">{f.status}</p></div></div>
        ...
        <ModalConfirmDelete open={!!deleteTarget} itemName={deleteTarget?.numero} onClose={()=>setDeleteTarget(null)} onConfirm={handleApagar} title="Apagar fatura?" description="Essa fatura será removida permanentemente." />
      </div>
    </div>
  )
}
