...
export default function EmitirFaturaPage() {
    ...
    // helper seguro - corrige o erro do slice
    const getNumero = (f: any) => f?.numero_fatura || f?.numero_proforma || f?.numero || f?.id?.slice(0, 8) || '---'
    const getTotal = (f: any) => Number(f?.total_geral ?? f?.total ?? 0)
    ...
    const emitir = async () => {
        ...
        try {
            ...
            toast.success('Fatura emitida'); setItens([]); setActiveTab('curso'); fetchFaturas()
        }
    ...
    return (
        <div className="min-h-screen bg-white">
            <div className="max-w-[1100px] mx-auto">
                ...
                <ModalConfirmDelete open={!!deleteTarget} itemName={deleteTarget ? getNumero(deleteTarget) : ''} onClose={() => setDeleteTarget(null)} onConfirm={handleApagar} title="Apagar fatura?" description="Essa fatura será removida permanentemente." />
            </div>
        </div>
    )
}
