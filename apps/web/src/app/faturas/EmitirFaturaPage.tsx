...
export default function EmitirFaturaPage() {
    ...
    // LIMPA URL SUJA da print:?tab=curso&cliente_id=xxx#/faturas/nova?cliente_id=xxx
    useEffect(() => {
        if (window.location.search) {
            // se tem query fora do hash, move pra dentro do hash e limpa
            ...
            if (hash &&!hash.includes('cliente_id=')) {
                ...
                // força reload dos searchParams do HashRouter
    ...
    // guarda tab sem causar loop #310
    useEffect(() => {
        if (!clienteId) return
