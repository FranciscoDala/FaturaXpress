...
export const FaturaPDF = ({ fatura, empresa, cliente, onClose }: Props) => {
  ...
  // IMPRIMIR SÓ A FOLHA - CORREÇÃO DO TEU BUG
  const handlePrint = () => {
    if (!folhaRef.current) return
    const printContent = folhaRef.current.innerHTML
    const win = window.open('', '', 'width=900,height=650')
    if (!win) return
    win.document.write(`
      <html><head><title>${getNumero(fatura)}</title>
      <style>
        @page { size: A4; margin: 0; }
        body { margin:0; padding:0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        #fatura-print { width:210mm; min-height:297mm; padding:10mm; font-family: Arial, sans-serif; box-sizing:border-box; }
        table { border-collapse: collapse; width:100%; }
        th, td { border: 1px solid #999; }
      </style>
      </head><body><div id="fatura-print">${printContent}</div></body></html>
    `)
    win.document.close()
    win.focus()
  ...
  // BAIXAR PDF DE VERDADE
  const handleBaixarPDF = async () => {
    setShowBaixar(false)
    // usa impressão como PDF - navegador vai salvar só a folha
    handlePrint()
  }
  ...
  const Toolbar = ({ overlay = false }: { overlay?: boolean }) => (
    <div className={`no-print w-full bg-white border-b border-gray-200 flex items-center justify-between px-3 md:px-4 py-2 ${overlay? 'fixed top-0 left-0 z-[100] shadow-md' : 'sticky top-0 z-50'}`}>
      <div className="flex items-center gap-2">
        ...
        <button className="w-9 h-9 hidden md:flex items-center justify-center rounded-full hover:bg-gray-100"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#5f6368" strokeWidth="1.8"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" /></svg></button>
        <button className="w-9 h-9 hidden md:flex items-center justify-center rounded-full hover:bg-gray-100"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#5f6368" strokeWidth="1.8"><circle cx="18" cy="5" r="3" /><circle cx="6" cy="12" r="3" /><circle cx="18" cy="19" r="3" /><path d="M8.6 13.5l6.8 4M15.4 6.5l-6.8 4" /></svg></button>
      </div>
  ...
  return (
    <>
      <style>{`
        #fatura-pdf-wrapper { display:flex; justify-content:center; background:white; width:100%; }
        ...
        /* CORREÇÃO DA IMPRESSÃO - SÓ IMPRIME A FOLHA */
        @media print {
          @page { size: A4; margin: 0; }
          html, body { margin:0!important; padding:0!important; background:white!important; }
          body * { visibility: hidden!important; }
          #print-area, #print-area * { visibility: visible!important; }
          #print-area { position: absolute!important; left:0!important; top:0!important; width:210mm!important; min-width:210mm!important; transform: none!important; margin:0!important; border:none!important; box-shadow:none!important; }
      ...
      <div id="fatura-pdf-wrapper" className="bg-white p-0 md:p-6">
        {/* ESSA DIV É A ÚNICA QUE VAI PRA IMPRESSÃO */}
        <div id="print-area">
          <div ref={folhaRef} id="fatura-pdf" className="relative bg-white text-black w-[210mm] min-w-[210mm] min-h-[297mm] p-[10mm] font-sans text-[12px] leading-[1.3] flex flex-col border border-gray-300 overflow-hidden mx-auto">
            ...
            <div className="relative z-10 flex flex-col flex-1">
              ...
              <div className="mt-auto pt-6 border-t border-black text-[8px]"><span>Licenciado a: {mask(emp.nome)} | NIF: {mask(emp.nif)} | Morada: {mask(emp.endereco)} | Data Impressão 15-09-2026 10:45 | Pág. 1 de 1</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      {isFullscreen && (
        <div className="fixed inset-0 bg-white z-[100] overflow-auto no-print">
          ...
          <div className="pt-[56px] bg-[#f8f9fa] min-h-screen flex justify-center p-2 md:p-6">
            <div className="origin-top scale-[0.5] md:scale-100">
              <div id="print-area">
                <div className="bg-white w-[210mm] min-h-[297mm] p-[10mm] border border-gray-300"> {/* clone simplificado pro fullscreen não precisa ref */} </div>
              </div>
            </div>
          </div>
        </div>
