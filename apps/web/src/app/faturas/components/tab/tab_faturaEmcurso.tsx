...
import { Trash2, XCircle, Eye, ArrowRight, Lock } from 'lucide-react'
import { toast } from 'sonner'
import { TabCursoSkeleton } from '../../../../components/CardsSkeleton'
import { api } from '../../../../lib/api'
import { getNumero, getTotal } from '../../page'
...
const CARGOS_PERMISSOES: Record<string, string[]> = {
    admin: ["*"],
    financeira: ["ver_faturas", "converter_fatura", "cancelar_fatura", "apagar_proforma"],
    rh: ["ver_funcionarios"],
...
export default function TabCurso({ faturas, cliente, empresa, onRefresh, loading = false }: Props) {
    ...
    const podeConverter = funcionarioLogado? temPermissao(cargoAtual, 'converter_fatura') || cargoAtual === 'admin' : true
    const podeCancelar = funcionarioLogado? temPermissao(cargoAtual, 'cancelar_fatura') || cargoAtual === 'admin' : true
    const podeApagar = funcionarioLogado? temPermissao(cargoAtual, 'apagar_proforma') || cargoAtual === 'admin' : true
    ...
    const handleAskConverter = (f: any) => {
        if (!podeConverter) { toast.error(`Cargo ${cargoAtual} não pode converter`); return }
        setConvertTarget(f)
    }
    const handleAskCancelar = (f: any) => {
        if (!podeCancelar) { toast.error(`Cargo ${cargoAtual} não pode cancelar`); return }
        setCancelTarget(f)
    }

    const handleConverter = async () => {
        if (!convertTarget) return
        if (!podeConverter) { toast.error('Sem permissão'); return }
        try {
            setConverting(true)
    ...
    const handleApagar = async () => {
        if (!podeApagar) { toast.error('Sem permissão'); return }
        try {
    ...
    if (!podeVer) {
        return (
            <div className="text-center py-16 bg-white rounded-[20px] border">
                ...
                <p className="text-[13px] text-gray-500">Seu cargo <b>{cargoAtual}</b> não pode ver proformas</p>
            </div>
        )
    }
    ...
    return (
        <div className="w-full px-4 sm:px-0 lg:px-0 mt-0">
            ...
            {faturasVisiveis.length === 0? (
                <p className="text-center text-gray-500 py-16 bg-white rounded-[20px] border">Nenhuma fatura proforma (FT/PP) {cargoAtual.toUpperCase()}</p>
            ) : (
                            ...
                            <div key={f.id} className="w-full min-w-[calc(100vw-32px)] md:min-w-[320px] md:max-w-[320px] snap-start flex-shrink-0 bg-white rounded-[22px] overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-gray-100 flex flex-col">
                                <div className="relative h-[90px] bg-[#FFF7CC] shrink-0">
                                    <div className="absolute top-3 right-3 bg-white px-3 py-1 rounded-full text-[10px] font-bold shadow-sm border text-[#8A6D00] truncate max-w-[70%]">
                                        PROFORMA PP {numero} {cargoAtual!== 'admin'? `• ${cargoAtual.toUpperCase()}` : ''}
                                    </div>
                                    ...
                                    {!podeConverter && <div className="absolute top-3 left-3 bg-black/70 text-white px-2 py-1 rounded-full text-[8px] font-bold">SOMENTE LEITURA</div>}
                                </div>
                                <div className="pt-14 px-5 pb-4 min-w-0 flex-1 overflow-hidden">
                                    ...
                                    <button disabled={!podeConverter} onClick={() => handleAskConverter(f)} className={`mt-3 w-full h-[38px] rounded-full text-[12px] font-bold flex items-center justify-center gap-1 shrink-0 ${podeConverter? 'bg-[#0095ff] text-white hover:bg-[#0080e0]' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
                                ...
                                <div className="grid grid-cols-3 border-t border-gray-100 mt-auto shrink-0">
                                    ...
                                    <button disabled={!podeCancelar} onClick={() => handleAskCancelar(f)} className={`py-3.5 flex justify-center border-x border-gray-100 transition group ${podeCancelar? 'hover:bg-gray-50' : 'bg-gray-50 opacity-40 cursor-not-allowed'}`}><XCircle className={`w-4 h-4 ${podeCancelar? 'text-gray-600 group-hover:text-orange-600' : 'text-gray-300'}`} /></button>
                                    <button disabled={!podeApagar} onClick={() => podeApagar && setDeleteTarget(f)} className={`py-3.5 flex justify-center transition group ${podeApagar? 'hover:bg-gray-50' : 'bg-gray-50 opacity-40 cursor-not-allowed'}`}><Trash2 className={`w-4 h-4 ${podeApagar? 'text-gray-600 group-hover:text-red-600' : 'text-gray-300'}`} /></button>
                                </div>
                            </div>
                        )
                    })}
            ...
            {podeApagar && <ModalConfirmDelete open={!!deleteTarget} itemName={deleteTarget? `PROFORMA PP ${cleanNumero(getNumero(deleteTarget))}` : ''} onClose={() => setDeleteTarget(null)} onConfirm={handleApagar} title="Apagar proforma?" description="Proforma PP pode ser apagada. É livre e não conta no limite. FT oficial só cancela - regra AGT." />}
            {podeConverter && <ModalConfirmConverter open={!!convertTarget} numero={convertTarget? cleanNumero(getNumero(convertTarget)) : ''} total={convertTarget? getTotal(convertTarget) : 0} loading={converting} onClose={() =>!converting && setConvertTarget(null)} onConfirm={handleConverter} />}
            {podeCancelar && <ModalConfirmCancelarPP open={!!cancelTarget} numero={cancelTarget? cleanNumero(getNumero(cancelTarget)) : ''} loading={canceling} onClose={() =>!canceling && setCancelTarget(null)} onConfirm={handleConfirmCancelar} />}
        </div>
    )
}
