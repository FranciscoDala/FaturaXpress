...
import { Plus, Trash2, Search, Star, ChevronDown, Check, ChevronLeft, ChevronRight, Crown, AlertTriangle } from 'lucide-react'
...
export default function TabEmitir({ clienteId, onEmitida }: { clienteId?: string; onEmitida: () => void }) {
    ...
    const emitir = async () => {
        if (!itens.length) return toast.error('Adicione produtos', { description: 'Selecione pelo menos 1 produto.' })
        if (!clienteId && !clienteNome) return toast.error('Informe nome do cliente avulso', { description: 'Campo nome é obrigatório para avulso.' })
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.8fr_1fr] gap-4 w-full">
                ...
                <div className="bg-white rounded-[22px] shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-gray-100 p-6 h-fit">
                    ...
                    <div className="border-t border-gray-100 pt-4 mt-4 text-[12.5px] space-y-2">
                        ...
                        {tipoDoc === 'fatura' && !isAtLimit && !isTudoIsento && <p className="text-[10px] font-semibold bg-green-50 border border-green-200 text-black rounded-full px-3 py-1 text-center mt-2">FT vai gerar Hash AGT + QR • Conta {usoMes + 1}/{planInfo.max || '∞'} neste mês</p>}
                        ...
                        <button
                            ...
                            className={`w-full mt-4 h-[46px] rounded-full font-semibold text-[13px] shadow-[0_4px_12px_rgba(0,149,255,0.25)] transition ${tipoDoc === 'fatura' && isAtLimit ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-[#0095ff] text-white hover:bg-[#0085e6]'}`}>
                            {tipoDoc === 'fatura' ? isAtLimit ? `Limite ${planInfo.max} FT atingido` : `Emitir FT Oficial (${usoMes + 1}/${planInfo.max || '∞'})` : 'Emitir Proforma PP (Livre)'}
                        </button>
                    </div>
                </div>
            </div>
