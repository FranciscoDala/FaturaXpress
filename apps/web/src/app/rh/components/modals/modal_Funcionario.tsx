...
// CALENDAR PERSONALIZADO IGUAL SAFT - MAS PARA DIA
function CustomDatePicker({ value, onChange, placeholder }: { value: string, onChange: (v: string)=>void, placeholder: string }) {
  ...
  const startDay = (new Date(view.year, view.month, 1).getDay()+6)%7 // segunda = 0
  ...
  return (
    <div ref={ref} className="relative w-full">
      <button type="button" onClick={()=> setOpen(!open)} className={`w-full h-[44px] bg-white border border-gray-200 rounded-[12px] px-3 text-[13.5px] flex items-center justify-between focus:outline-none focus:border-[#0095ff] transition ${value? 'text-black' : 'text-black/40'}`}>
      ...
      {open && (
        <div className="absolute z-[9999] top-[48px] left-0 w-full bg-white rounded-[20px] shadow-[0_16px_48px_rgba(0,0,0,0.18)] border border-gray-100 overflow-hidden">
          <div className="h-[56px] px-4 flex items-center justify-between bg-[#F8FAFF] border-b border-gray-100">
            ...
            <span className="text-[14px] font-bold text-gray-900">{MONTH_LABEL[view.month]} {view.year}</span>
          ...
          <div className="p-3">
            ...
            <div className="grid grid-cols-7 gap-1">
              {days.map((day, idx)=> day===null? <div key={`empty-${idx}`} /> : (
                <button key={`${view.year}-${view.month}-${day}`} type="button" onClick={()=> selectDay(day)} className={`h-9 rounded-[10px] text-[13px] font-medium transition ${isSelected(day)? 'bg-[#0A2540] text-white border border-[#0A2540]' : 'bg-white border border-gray-100 hover:bg-gray-50 text-gray-700'}`}>
                  {day}
                </button>
              ))}
            </div>
            <div className="mt-3 flex gap-2">
              <button type="button" onClick={()=> { onChange(""); setOpen(false) }} className="flex-1 h-9 rounded-full border border-gray-200 text-[12px] text-gray-500">Limpar</button>
              <button type="button" onClick={()=> {
                const today = new Date()
                const iso = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`
                onChange(iso); setOpen(false)
              }} className="flex-1 h-9 rounded-full bg-[#E6F0FF] border border-[#C2D8FF] text-[12px] font-semibold text-[#0095ff]">Hoje</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[24px] w-full max-w-[560px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.25)] max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 pt-5 pb-3 shrink-0 border-b border-gray-100">
          ...
          <p className="text-[13.5px] text-gray-500 mt-1">Cadastro completo via BI - calendar personalizado</p>
