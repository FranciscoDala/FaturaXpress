...
// SELECT CARD PADRAO UNICO - QUADRADO FINO
    ...
    return (
        <div ref={ref} className="relative w-full">
            <button
                ...
                className={`w-full h-[44px] flex items-center justify-between px-3 bg-white/90 backdrop-blur border border-gray-200 rounded-[3px] text-[13px] font-medium text-left shadow-sm transition ${disabled ? 'opacity-50 cursor-not-allowed bg-gray-50' : 'hover:bg-white'} ${open ? 'border-[#0095ff] ring-1 ring-[#0095ff]/20' : ''}`}
            >
                <span className="truncate text-gray-800">{selected?.label || 'Selecione'}</span>
                <ChevronDown className={`w-4 h-4 text-gray-400 shrink-0 transition ${open ? 'rotate-180' : ''}`} />
            </button>
            {open && (
                <div className="absolute z-20 top-[48px] left-0 w-full bg-white border border-gray-200 rounded-[3px] overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
                    {options.map(opt => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full h-[40px] px-3 flex items-center justify-between text-[13px] text-left border-b last:border-b-0 border-gray-100 transition ${active ? 'bg-[#F0F7FF] text-[#0095ff] font-bold' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
                            >
                                {opt.label}
...
export default function ModalGestaoFalta({ data, open, onClose, onSaved, dataSelecionada, usuario }: { data: AuditData | null, open: boolean, onClose: () => void, onSaved: () => void, dataSelecionada: string, usuario?: any }) {
    ...
    const TabButton = ({ id, label, icon: Icon }: { id: any, label: string, icon: any }) => (
        <button type="button" onClick={() => setTab(id)} className={`flex items-center gap-1.5 px-3 py-2 text-[13px] font-medium rounded-[3px] transition border shrink-0 ${tab === id ? 'bg-[#F0F7FF] border-[#C2D8FF] text-[#0095ff]' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}><Icon className="w-4 h-4" />{label}</button>
    )
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[24px] w-full max-w-[460px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.25)] max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="px-6 pt-5 pb-3 shrink-0 border-b border-gray-100">
                    ...
                    {!podeJustificar && (
                        <div className="mt-2 p-2 rounded-[3px] bg-red-50 border border-red-200 flex items-center gap-2 text-[11px] text-red-700"><Lock className="w-3.5 h-3.5" /> Sem permissão</div>
                    )}
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-4">
                    ...
                    {tab === 'detalhes' && (
                        <div className="flex flex-col gap-2">
                            <div className="p-3 rounded-[3px] bg-white border shadow-sm text-[13px] text-black"><b>Lançado por:</b> {data.lancado_por_nome || 'RH'} • cargo: {cargoAtual}</div>
                            <div className="p-3 rounded-[3px] bg-amber-50 border border-amber-200 text-[13px] text-amber-900">{data.motivo_retroativo || data.motivo}</div>
                            {falta.justificativa_tipo && <div className="p-3 rounded-[3px] bg-blue-50 border border-blue-200 text-[13px]"><b>{falta.justificativa_tipo}</b> • {falta.justificativa_obs}</div>}
                        </div>
                    )}
                    {tab === 'justificar' && (
                        <div className="flex flex-col gap-3">
                            {/* SELECT NO PADRAO CARD QUADRADO */}
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                {file && file.type.startsWith('image/') && previewUrl && <img src={previewUrl} alt="prev" className="w-full max-h-[160px] object-contain rounded-[3px] border shadow-sm" />}
                            </div>
                            <textarea disabled={!podeJustificar} value={obs} onChange={e => setObs(e.target.value)} placeholder="Observação" className="w-full min-h-[80px] bg-white border border-gray-200 rounded-[3px] p-3 text-[13.5px] resize-none shadow-sm disabled:bg-gray-50 focus:outline-none focus:border-[#0095ff] focus:ring-1 focus:ring-[#0095ff]/20" />
                        </div>
                    )}
                    {tab === 'gestao' && (
                        <div className="flex flex-col gap-3">
                            {!podeAprovar && <div className="p-2 rounded-[3px] bg-white border shadow-sm text-[11px] text-gray-600 flex items-center gap-2"><Lock className="w-3.5 h-3.5" /> Só admin/RH pode aprovar/rejeitar</div>}
                            {falta.justificativa_anexo_url && (
                                <div className="p-3 rounded-[3px] bg-white border shadow-sm">
                                    ...
                                    {falta.justificativa_anexo_url.includes('pdf') || falta.justificativa_anexo_url.startsWith('data:application') ?
                                        <a href={falta.justificativa_anexo_url} target="_blank" rel="noreferrer" className="w-full h-[44px] bg-black text-white rounded-[3px] flex items-center justify-center gap-2 text-[12px] font-bold"><Eye className="w-4 h-4" /> Abrir PDF</a> :
                                        <img src={falta.justificativa_anexo_url} alt="doc" className="w-full max-h-[240px] object-contain rounded-[3px] border" />}
                                </div>
                            )}
                ...
                <div className="shrink-0 px-6 py-4 border-t bg-white flex gap-2">
                    {tab === 'justificar' ? (
                        <>
                            <button type="button" onClick={() => setTab('detalhes')} className="flex-1 h-11 rounded-[3px] border bg-white flex items-center justify-center shadow-sm"><X className="w-5 h-5" /></button>
                            <button type="button" disabled={loading || !podeJustificar} onClick={uploadAndJustificar} className="flex-1 h-11 rounded-[3px] bg-[#0095ff] text-white flex items-center justify-center disabled:opacity-50 shadow-sm">{loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Check className="w-5 h-5" />}</button>
                        </>
                    ) : tab === 'gestao' && isAdmin ? (
                        <>
                            <button type="button" onClick={onClose} className="flex-1 h-11 rounded-[3px] border bg-white flex items-center justify-center shadow-sm"><X className="w-5 h-5" /></button>
                            {falta.status === 'pendente_justificacao' && <>
                                <button type="button" disabled={!podeRejeitar} onClick={handleRejeitar} className={`w-11 h-11 rounded-[3px] border flex items-center justify-center shadow-sm ${podeRejeitar ? 'bg-red-50 border-red-200' : 'bg-gray-100 opacity-40 cursor-not-allowed'}`}><Ban className="w-5 h-5 text-red-600" /></button>
                                <button type="button" disabled={!podeAprovar} onClick={handleAprovar} className={`flex-1 h-11 rounded-[3px] text-white flex items-center justify-center shadow-sm ${podeAprovar ? 'bg-[#00c950]' : 'bg-gray-300 cursor-not-allowed'}`}><Check className="w-5 h-5" /></button>
                            </>}
                            {falta.abonada && <button type="button" disabled={!podeRemover} onClick={handleRemover} className={`w-11 h-11 rounded-[3px] text-white flex items-center justify-center shadow-sm ${podeRemover ? 'bg-black' : 'bg-gray-300 cursor-not-allowed'}`}><Trash2 className="w-5 h-5" /></button>}
                        </>
                    ) : (
                        <>
                            <button type="button" onClick={onClose} className="flex-1 h-11 rounded-[3px] border bg-white flex items-center justify-center shadow-sm"><X className="w-5 h-5" /></button>
                            {podeJustificar && <button type="button" onClick={() => setTab('justificar')} className="flex-1 h-11 rounded-[3px] bg-[#0095ff] text-white flex items-center justify-center shadow-sm"><Upload className="w-5 h-5" /></button>}
                        </>
                    )}
                </div>
            </div>
