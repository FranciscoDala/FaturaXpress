...
export default function RelatorioAuditoriaPonto({ dataSelecionada, pontos, faltas, funcs, minDate, hoje, onClose }: Props){
    const [empresa, setEmpresa] = useState<any>(null)

    useEffect(()=>{
        // CARREGA EMPRESA EM TEMPO REAL - SEM QUEBRAR SE DER 404
        const loadEmp = async () => {
            try{ const r = await api.get('/api/empresa'); setEmpresa(r.data); return }catch{}
            try{ const r = await api.get('/api/empresas/me'); setEmpresa(r.data); return }catch{}
            try{ const r = await api.get('/api/config'); setEmpresa(r.data?.empresa || r.data); return }catch{}
            const stored = localStorage.getItem('empresa') || localStorage.getItem('empresa_logada')
            if(stored){ try{ setEmpresa(JSON.parse(stored)) }catch{} }
        }
        loadEmp()
    },[])
    ...
    const linhas = useMemo(()=>{
        ...
        return funcs.map(func=>{
            ...
            return { nome: func.nome, entrada: entrada? fmtHora(entrada.timestamp) : '---', saida: saida? fmtHora(saida.timestamp) : '---', status, motivo, retro: isRetro? 'SIM' : 'NAO', quem: entrada?.lancado_por_nome || saida?.lancado_por_nome || falta?.lancado_por_nome || (lista.length? 'Proprio' : 'RH'), lanc: entrada?.lancado_em || falta?.lancado_em? fmtHora(entrada?.lancado_em || falta?.lancado_em) : '---' }
        })
    },[funcs,pontos,faltas])
    ...
    return (
        <div className="fixed inset-0 z-[10000] bg-white overflow-y-auto">
            ...
            <div id="relatorio" className="w-[210mm] min-h-[297mm] mx-auto p-[12mm] bg-white text-black font-[Arial] border">
                {/* HEADER EMPRESA - PROFISSIONAL SEM COR */}
                <div className="flex justify-between items-start border-b-2 border-black pb-4">
                    <div className="flex gap-4">
                        {empresa?.logo_url || empresa?.logo? <img src={empresa.logo_url || empresa.logo} className="w-[70px] h-[70px] object-contain border"/> : <div className="w-[70px] h-[70px] border border-black flex items-center justify-center font-bold text-[20px]">{empresa?.nome?.charAt(0) || 'E'}</div>}
                        <div className="text-[11px] leading-[15px]">
                            <p className="font-bold text-[14px] uppercase">{empresa?.nome || 'Carregando empresa...'}</p>
                            <p>NIF: {empresa?.nif || '---'}</p>
                            <p>{empresa?.endereco || empresa?.morada || '---'}</p>
                            <p>Tel: {empresa?.telefone || empresa?.contactos || '---'} | Email: {empresa?.email || '---'}</p>
                            <p>{empresa?.cidade || ''}</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <p className="font-bold text-[12px] border border-black px-3 py-1">AUDITORIA DE PONTO</p>
                        <p className="text-[10px] mt-2">Data: {fmtDisplay(dataSelecionada)}</p>
                        <p className="text-[9px]">Emissao: {new Date().toLocaleString('pt-AO')}</p>
                    </div>
                </div>

                <div className="mt-4 text-[10px] flex justify-between">
                    <p><b>Periodo retroativo:</b> {fmtDisplay(minDate)} ate {fmtDisplay(hoje)}</p>
                    <p><b>Total funcionarios:</b> {funcs.length} | <b>Presentes:</b> {linhas.filter(l=>l.status!=='FALTA' && l.status!=='SEM REGISTO').length} | <b>Faltas:</b> {linhas.filter(l=>l.status==='FALTA').length}</p>
                </div>

                <table className="w-full mt-4 border-collapse border border-black text-[10px]">
                    <thead>
                        <tr className="bg-black text-white">
                            <th className="border border-black p-1.5 text-left w-[26%]">FUNCIONARIO</th>
                            <th className="border border-black p-1.5 w-[10%]">ENTRADA</th>
                            <th className="border border-black p-1.5 w-[10%]">SAIDA</th>
                            <th className="border border-black p-1.5 w-[12%]">STATUS</th>
                            <th className="border border-black p-1.5 text-left w-[20%]">MOTIVO / ATRASO</th>
                            <th className="border border-black p-1.5 w-[8%]">RETRO?</th>
                            <th className="border border-black p-1.5 text-left w-[14%]">QUEM LANCOU</th>
                        </tr>
                    </thead>
                    <tbody>
                        {linhas.map((l,i)=>(
                            <tr key={i} className="h-[26px]">
                                ...
                                <td className="border border-black text-center font-bold">{l.status}</td>
                ...
                <div className="mt-6 text-[9px] leading-[13px] border-t border-black pt-3 flex justify-between">
                    <div><p>Documento gerado automaticamente pelo sistema de ponto.</p><p>Conversao: 177min = 2h57min atraso | 308min = 5h08min atraso</p></div>
                    <div className="text-right"><p>Assinatura RH: _________________________</p><p className="mt-4">Data: {new Date().toLocaleDateString('pt-AO')}</p></div>
                </div>
            </div>
        </div>
    )
