...
export const getAlertStyle = (n: any) => {
    ...
    const dataFalta = n.falta?.data_inicio || n.falta?.data || n.created_at

    // RETORNO DO ADMIN PARA O RH - verde/azul de feedback
    if (isRetornoAdmin) {
        if (['aprovada', 'aprovado', 'justificado', 'abonada'].includes(s)) {
            return {
                ...
                label: `Admin aprovou • falta ${formatarDataCurta(dataFalta)}`
            }
        }
        if (['rejeitada', 'rejeitado'].includes(s)) {
            return {
                ...
                label: `Admin rejeitou • falta ${formatarDataCurta(dataFalta)}`
            }
        }
    }
    ...
    if (['aprovada', 'aprovado', 'justificado', 'abonada'].includes(s)) {
        return isAtraso
           ? { bg: 'bg-green-50', border: 'border-green-200', badge: 'bg-green-100 text-green-800 border-green-200', label: 'Falta por atraso aplicada' }
            : { bg: 'bg-green-50', border: 'border-green-200', badge: 'bg-green-100 text-green-800 border-green-200', label: isRetornoAdmin? 'Aprovada pelo Admin' : 'Falta justificada' }
    }
