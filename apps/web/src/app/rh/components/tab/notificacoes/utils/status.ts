...
export const getAlertStyle = (n: any) => {
    ...
    if (isRetornoAdmin) {
        if (['aprovada', 'aprovado', 'justificado', 'abonada'].includes(s)) {
            return { bg: 'bg-green-50', border: 'border-green-300', badge: 'bg-green-100 text-green-800 border-green-300', label: `Admin aprovou • ${formatarDataCurta(n.falta?.data_inicio || n.created_at)}` }
        }
        if (['rejeitada', 'rejeitado'].includes(s)) {
            return { bg: 'bg-red-50', border: 'border-red-300', badge: 'bg-red-100 text-red-700 border-red-300', label: `Admin rejeitou • ${formatarDataCurta(n.falta?.data_inicio || n.created_at)}` }
        }
    }
