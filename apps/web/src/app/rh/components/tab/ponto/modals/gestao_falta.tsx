...
export default function ModalGestaoFalta({ data, open, onClose, onSaved, dataSelecionada, usuario }: { data: AuditData | null, open: boolean, onClose: () => void, onSaved: () => void, dataSelecionada: string, usuario?: any }) {
    ...
    const cargoAtual = (funcionarioLogado?.cargo || usuario?.cargo || 'rh').toLowerCase()
    const podeJustificar = useMemo(() => {
        if (cargoAtual === 'admin') return true
        return temPermissao(cargoAtual, 'justificar_falta') || temPermissao(cargoAtual, 'justificar_falta_propria')
    }, [cargoAtual])
    ...
    useEffect(() => {
        if (open) {
            ...
            if (data) {
                ...
                setFile(null)
                setTab(data.justificativa_tipo || data.justificativa_anexo_url? 'justificar' : 'detalhes')
            } else {
                setTab('detalhes')
            }
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!['application/pdf', 'image/jpeg', 'image/png', 'image/jpg', 'image/webp'].includes(f.type)) { toast.error('Só PDF ou imagem'); return }
        setFile(f)
    ...
    // apiRoot já é https://.../api, então fica /api/rh/falta/...
        ...
        try {
            ...
            if (file) {
                ...
                try {
                    ...
                    const up = await api.post('/upload/falta', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
                    anexoUrl = (up.data as any).url || (up.data as any).file_url
                    if (!anexoUrl) throw new Error('URL não retornada')
                } catch (upErr: any) {
                    const d = upErr?.response?.data?.detail || upErr?.response?.data?.message || upErr?.message
                    console.error('[UPLOAD] ERRO:', upErr?.response?.data)
            ...
            // CORRIGIDO: sem /api no início
            await api.post(`/rh/falta/${falta.id}/justificar`, {
                tipo: docNome.trim(),
            ...
            const detail = e?.response?.data?.detail || e?.response?.data?.msg || JSON.stringify(e?.response?.data) || e?.message || 'Erro desconhecido'
            console.error('[JUSTIFICAR] ERRO:', e?.response?.data, e?.response?.status)
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[24px] w-full max-w-[460px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.25)] max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-y-auto overflow-x-hidden no-scrollbar px-6 py-4">
                    ...
                    {tab === 'detalhes' && (
                        <div className="flex flex-col gap-2">
                            <div className="p-3 rounded-[12px] bg-gray-50 border border-gray-200 flex flex-col gap-1.5">
                                <div className="flex justify-between text-[13px]"><span className="text-black/60">Funcionário</span><span className="font-bold text-black truncate max-w-[160px]">{data.funcionario_nome || data.funcionario?.nome || '—'}</span></div>
                                ...
                                <div className="flex justify-between text-[13px]"><span className="text-black/60">Status</span><span className="font-bold text-black uppercase text-[11px]">{data.status}</span></div>
                            ...
                            {falta.justificativa_tipo && (
                                <div className="p-3 rounded-[12px] bg-[#F0F7FF] border border-[#C2D8FF]">
                                    ...
                                    {falta.justificativa_anexo_url && (
                                        <a href={comprovanteEndpoint} target="_blank" rel="noreferrer" className="mt-3 w-full h-[44px] bg-black text-white rounded-full flex items-center justify-center gap-2 text-[13px] font-bold"><Eye className="w-4 h-4" /> Ver Comprovante</a>
                                    )}
                                </div>
                            )}
                        </div>
                    ...
                    {tab === 'justificar' && (
                        <div className="flex flex-col gap-2">
                            <input type="text" value={docNome} onChange={e => setDocNome(e.target.value)} placeholder="Ex: Atestado Médico" disabled={!podeJustificar} className={inputClass} />
                            <input ref={fileRef} type="file" accept="application/pdf,image/*" onChange={handleFileChange} className="hidden" />
                            <button type="button" disabled={!podeJustificar} onClick={() => fileRef.current?.click()} className="w-full min-h-[88px] border-2 border-dashed rounded-[12px] flex flex-col items-center justify-center gap-1.5 bg-white border-gray-300 hover:border-[#0095ff] transition px-2 py-3">
                                <Upload className="w-5 h-5 text-black" />
                                <span className="text-[12.5px] font-bold text-black text-center w-full truncate max-w-[300px] px-2 block">{file? file.name : 'Carregar PDF ou imagem'}</span>
                                <span className="text-[11px] text-black/60">PDF, JPG, PNG - máx 5MB</span>
                            </button>
                            {file?.type.startsWith('image/') && previewUrl && <img src={previewUrl} alt="prev" className="w-full max-h-[160px] object-contain rounded-[12px] border border-gray-200" />}
                            ...
                            {showObs && <textarea disabled={!podeJustificar} value={obs} onChange={e => setObs(e.target.value)} placeholder="Observação opcional..." className="w-full min-h-[80px] bg-white border border-gray-200 rounded-[12px] px-3 py-2.5 text-[13.5px] text-black placeholder:text-black/60 focus:outline-none focus:border-[#0095ff] focus:ring-1 focus:ring-[#0095ff]/20 resize-none" />}
                        </div>
                    )}
                </div>
                ...
                <div className="shrink-0 px-6 py-4 border-t border-gray-100 bg-white flex gap-2">
                    {tab === 'justificar'? (
                        <>
                            <button type="button" onClick={() => setTab('detalhes')} className="flex-1 h-11 rounded-full border border-gray-200 bg-white flex items-center justify-center hover:bg-gray-50 transition"><X className="w-5 h-5 text-black" /></button>
                            <button type="button" disabled={loading ||!podeJustificar} onClick={uploadAndJustificar} className="flex-1 h-11 rounded-full bg-[#0095ff] text-white font-semibold hover:bg-[#0085e6] shadow-[0_6px_20px_rgba(0,149,255,0.35)] flex items-center justify-center disabled:opacity-50 transition">{loading? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-5 h-5" />}</button>
                        </>
                    ) : (
                        <>
