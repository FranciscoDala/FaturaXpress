              ...
              <div className="flex flex-row justify-between items-start gap-3 w-full">
                <div className="flex flex-col items-start text-left flex-1 min-w-0">
                  ...
                  <div className="mt-4 space-y-0 w-full">
                    <p className="text-[11px] text-gray-500">Funcionários - 12 registados</p>
                    <p className="text-[11px] text-gray-500">Ativos - <span className="text-[#22c55e] font-bold text-[13px]">10 ativos</span></p>
                    <p className="text-[11px] text-gray-600 font-medium">Férias - 2 este mês</p>
                  </div>
                </div>
        ...
        <div className="px-4 sm:px-8 lg:px-12 py-6 space-y-3">
          {FUNC_MOCK.map(f => (
            <div key={f.id} className="flex items-center gap-3 p-3 rounded-[16px] border border-gray-100 bg-white hover:shadow-[0_2px_12px_rgba(0,149,255,0.08)] transition">
              ...
              <span className={`text-[10px] px-2.5 py-1 rounded-full border font-medium ${f.status === 'ativo'? 'bg-green-50 text-green-600 border-green-100' : 'bg-yellow-50 text-yellow-600 border-yellow-100'}`}>{f.status}</span>
            </div>
          ))}
        </div>
      </div>
